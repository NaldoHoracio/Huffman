...
int main()
{
    ...
    first_byte_header = size_file[1]; // first_byte_header recebe o primeiro byte do cabeçalho
    ...
    first_byte_header_aux = first_byte_header << 3;
    int vector_of_size_tree[13], size_of_tree = 0;
    for( i = 0; i < 5; i++)
    {
        if( is_bit_i_set(first_byte_header, i))
            vector_of_size_tree[i] = 1;
        else
            vector_of_size_tree[i] = 0;
    }
    secund_byte_header = size_file[2];
    for( i = 0; i < 8; i++)
    {
        if( is_bit_i_set(secund_byte_header, i))
            vector_of_size_tree[i + 5] = 1;
        else
            vector_of_size_tree[i + 5] = 0;
    }
    int exp_tree;
    exp_tree = 12;
    for( i = 0; i < 13; i++)
    {
        size_of_tree = size_of_tree + (vector_of_size_trash[i] * (int) exp2(exp_tree));
        exp_tree--;
    }
