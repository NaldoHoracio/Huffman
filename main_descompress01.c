...
int main()
{
    ...
    for( i = 0; i < 5; i++)
    {
        if( is_bit_i_set(first_byte_header_aux, aux_size_tree))//Loop para verificar os 5 ultimos bits - verifica da esquerda para a direita
        {
        ...
        aux_size_tree--;// Decrementa para continuar a verificação, até o bit 0
    }
    secund_byte_header = size_file[1];// Recebendo o segundo byte do cabeçalho que complementa o tamanho da árvore
    aux_size_tree = 7;
    for( i = 0; i < 8; i++)
    {
        if( is_bit_i_set(secund_byte_header, aux_size_tree))// Verifica bit a bit do segundo byte se este está setado - da esquerda para a direita
